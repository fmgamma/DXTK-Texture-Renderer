...
//convert to and from 2d and 3d coords
...
//I want to reverse a transformation
DirectX::SimpleMath::Matrix InverseTranspose(const DirectX::SimpleMath::Matrix& m);

//get a random range min-max inclusive, either floats or ints
float GetRandom(float min, float max);
inline int GetRandom(int min, int max)
{
	return (int)round(GetRandom((float)min, (float)max));
}
//don't forget to seed the random generator
//if seed is left to default to -1 then the clock is used to seed the generator
